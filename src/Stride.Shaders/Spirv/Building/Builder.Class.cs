...
public partial class SpirvBuilder
{
    ...
    private static void TransformResolvedSemantics(NewSpirvBuffer shader, Dictionary<string, string> semantics)
    {
        for (var index = 0; index < shader.Count; index++)
        {
            var i = shader[index];
            if (
                i.Op == Op.OpDecorateString && (OpDecorateString)i is { Decoration: Decoration.UserSemantic, Value : string n } decorate
                && semantics.TryGetValue(n, out var newSemantic)
            )
            {
                decorate.Value = newSemantic;
            }
            else if (
                i.Op == Op.OpMemberDecorateString && (OpMemberDecorateString)i is { Decoration: Decoration.UserSemantic, Value: string n2 } decorate2
                && semantics.TryGetValue(n2, out var newSemantic2)
            )
            {
